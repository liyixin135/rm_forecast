...
void SpinObserver::update(rm_msgs::TrackData &target_msg,
                          ros::Time &current_time, double &max_jump_angle,
  ...
  if (1) {
    ...
    if (std::abs(yaw_diff) > max_jump_angle) {
      ...
      if (jump_count_ > 1 &&
          std::signbit(yaw_diff) == std::signbit(last_jump_yaw_diff_)) {
  ...
  /***是否开火***/
  ROS_INFO("allow_following_range:%f", time_after_jumping / jump_period_);
  if (target_spinning_) {
